...
export default function AdminFreeform({ onNavigate, selectedProject, onProjectChange, activities, currentUser }) {
  const [activeTab, setActiveTab] = useState("anomali"); // "anomali" | "dinamis"
      ...
      <div className="p-4 md:p-8 space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1 text-sm">Kelola aturan anomali spesifik per kegiatan dan grup pilihan dinamis untuk Form Builder.</p>
          </div>
        ...
        {/* Tabs */}
        <div className="flex space-x-1 p-1 bg-slate-100 rounded-xl w-max">
          <button
            onClick={() => setActiveTab("anomali")}
            className={`px-5 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 transition-all cursor-pointer ${
              activeTab === "anomali" ? "bg-white text-blue-700 shadow-sm" : "text-slate-500 hover:text-slate-700"
            }`}
          >
          ...
          <button
            ...
            className={`px-5 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 transition-all cursor-pointer ${
              activeTab === "dinamis" ? "bg-white text-blue-700 shadow-sm" : "text-slate-500 hover:text-slate-700"
            }`}
          >
            <CheckSquare size={16} />
            Pilihan Dinamis
